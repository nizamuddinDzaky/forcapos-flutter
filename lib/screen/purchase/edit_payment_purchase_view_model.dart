...
import 'edit_payment_purchase_screen.dart';

abstract class EditPaymentPurchaseViewModel extends State<EditPaymentPurchaseScreen> {
  ...
  actionPutPayment(Map<String, String> body) async {
    var params = {
      MyString.KEY_PURCHASE_ID : payment.id
    };
    var status = await ApiClient.methodPut(
      ApiConfig.urlEditPaymentPurchase,
      body,
      params,
      onBefore: (status) {},
      onSuccess: (data, _) {
  ...
  submitChanges() async {
    ...
    if (amount > 0) {
      Map<String, String> body = {
        ...
        'date': date.toStr(),
        'paid_by': paymentType[1].toString(),
